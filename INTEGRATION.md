...
already enabled (`app.use(cors())`), so fetch calls to `http://https://queue-backend-82a8.onrender.com`
succeed even from a `file://` page.

If you'd rather serve them from the backend itself (so they're reachable at
`http://https://queue-backend-82a8.onrender.com/kiosk.html` etc. — cleaner for a demo), ask Member 4 to
...
`http://https://queue-backend-82a8.onrender.com/kiosk.html` if you added the static line above).
...
`http://https://queue-backend-82a8.onrender.com`. If you deploy the backend (Render/Railway per the
